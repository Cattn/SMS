...
export const load: PageServerLoad = async ({ url }) => {
    ...
    try {
        const foldersResponse = await fetch('http://localhost:5823/api/folders/tree?includeExcluded=true');
        const foldersData = await foldersResponse.json();
        
        const filesUrl = new URL('http://localhost:5823/api/getFilesInPath');
        if (currentPath) {
