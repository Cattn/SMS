	...
	async cancelFileDeletion(filename: string): Promise<boolean> {
		try {
			...
			if (row) {
				...
				await db.run('UPDATE scheduled_deletions SET status = "CANCELLED" WHERE id = ?', [row.id]);
	...
	private async executeFileDeletion(id: string, filename: string, filePath: string) {
		try {
			if (fs.existsSync(filePath)) {
				...
				await db.run('UPDATE scheduled_deletions SET status = "COMPLETED" WHERE id = ?', [id]);
			...
			await db.run('UPDATE scheduled_deletions SET status = "FAILED" WHERE id = ?', [id]);
		...
		try {
			...
			for (const row of rows) {
				...
				if (scheduledAt <= now) {
					...
					console.log(
						`Restored timer for ${row.filename} (${Math.round(delay / 1000 / 60)} minutes remaining)`
					);
				}
			}
		} catch (error) {
